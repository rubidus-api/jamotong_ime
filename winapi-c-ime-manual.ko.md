...
*최종 갱신: 2026-07-24 (v0.14.0 — TRANSITORY 분기 §13.0·메모장 인라인 실기 PASS·밑줄 커서 오버레이 §12.1·§2.2/§8 최종 정리)*
...
- **최종 정리(2026-07-24)**: 이 3부류를 실행 시점에 가르는 실용 신호는 앱 이름이 아니라
  `ITfContext::GetStatus`의 **`TS_SS_TRANSITORY`**다(§12.7.8에서 확정, §13.0에서 제품 분기).
  단명(transitory) 컨텍스트=조합이 키마다 종료되는 것이 정상 → 커밋 전용, 비단명=표준
  composition 가능. 실기: 메모장 인라인 밑줄 조합 **PASS(2026-07-24)**, AkelPad는 단명
  판정으로 커밋 경로 유지.
...
- **DLL 잠금은 재부팅할 이유가 아니다**: 실행 중인 앱(`ctfmon`, 탐색기 …)이 매핑한 DLL 은 지우거나 덮어쓸 수
  없지만 **이름은 바꿀 수 있다**. 옆으로 옮기고 새 파일을 넣으면, 실행 중인 앱은 끝날 때까지 옛 사본을 쓴다.
- **32비트 등록은 32비트 보기로 확인한다**: `reg query "HKCR\CLSID\{...}\InprocServer32" /ve /reg:32`
  (64비트는 `/reg:64`). 스위치가 없으면 64비트 `reg.exe` 는 64비트 보기만 본다.
- **나중 정리가 재설치를 지우면 안 된다**: 제거기가 잠긴 잔여물 삭제를 예약한다면(`RunOnce`) `*.old.*` 와
  **빈** 폴더만 지우게 하고(`del ... & rd <폴더>`, `/s` 없이), 설치기는 그 항목을 지운다 — 재설치 전에 걸린
  `rd /s /q` 는 다음 로그인 때 새 설치를 통째로 지운다.
- **등록 경로가 옮겨진 뒤에는** 입력 표시기가 탐색기를 다시 시작하거나 다시 로그인할 때까지 캐시된 아이콘(또는
  "한글" 같은 언어 글자)을 보여 준다.
...
> **2026-07-24 갱신 — 이 장의 결론은 절반만 살아남았다.** "모든 호스트 커밋 전용"은 이후
> 실기(§12.7.8)로 **단명(`TS_SS_TRANSITORY`) 호스트 부류의 정답**으로 지위가 좁혀졌고,
> 비단명 호스트(메모장류)는 표준 composition이 성립해 제품이 런타임 분기한다(§13.0).
> 이 장은 그 결론에 도달한 과정과 단명 부류의 실기 증거로서 그대로 보존한다.
...
### 9.5 무간섭(직접 입력) 모드 — 원격 데스크톱과 로컬 IME의 구조적 충돌 (2026-07-24)

- **문제**: TSF IME는 로컬 포커스 창의 입력 스택 맨 앞에 선다. 원격 데스크톱 클라이언트
  (구글 원격 데스크톱 등)에 포커스가 있어도 로컬 IME가 자모키·자판 전환키를 먼저 소비하면
  원격 PC에는 keydown 자체가 도달하지 않는다 — 로컬만 전환되고 원격 한글 입력은 불가능하다.
  모든 로컬 IME가 공유하는 구조적 한계다(자동 감지는 원격 클라이언트가 브라우저 탭이라
  창 제목 휴리스틱밖에 없어 기각).
- **해법**: 명시적 **무간섭 모드** — 켜져 있는 동안 해제 단축키(설정 가능, 기본 없음) 외
  **모든 키를 통과**시킨다. 자판 전환키도 통과되므로 원격 쪽 IME가 받아 전환한다.
- **구현 요점**: TIP 인스턴스는 프로세스별이므로 상태 원본은 HKCU 레지스트리(DWORD)이고
  각 인스턴스는 캐시를 `OnSetFocus`에서 재읽는다(다른 프로세스에서 토글해도 따라감).
  토글 진입 시 조합 플러시·팝업 정리(모드 경계=조합 경계), 언어바/트레이 아이콘은 자판
  대신 `--`를 그려 상태를 보이게 한다. 우클릭 메뉴에 체크 표시 항목, 단축키는 SC_FN 테이블
  기반이라 설정창·저장/로드에 자동 편입된다.
...
- **후보창 키 라우팅을 TSF 키 싱크에만 의존하지 마라**: 일부 터미널(PuTTY, 실기 2026-07-24)은
  후보창이 뜬 상태에서 키를 싱크로 넘기지 않는다 — 마우스만 동작. 후보창 **표시 중에만**
  `WH_KEYBOARD_LL` 훅으로 탐색 키를 처리·차단하고(합성 입력 `LLKHF_INJECTED` 통과, 실제 처리는
  `PostMessage`로 넘겨 훅 콜백은 즉시 반환), 숨김과 함께 해제한다. 정상 호스트의 싱크 경로와는
  키 집합이 서로소라 이중 처리가 없다.
- **팝업은 모니터 작업영역으로 클램프하라**: 캐럿이 화면 가장자리면 후보창이 밖으로 잘린다
  (실기 2026-07-24). `MonitorFromPoint`+`rcWork`로 x를 클램프하고, 아래로 넘치면 캐럿 줄
  '위'로 뒤집는다.
...
### ★TIP 이 로드된 프로세스에서 프로세스 전역 DPI API 를 부르지 마라 (2026-08-24)

설정창 스레드가 `SetProcessDpiAwarenessContext(PER_MONITOR_AWARE_V2)` 를 불렀다 — TIP 의 창은
**호스트 프로세스 안**에서 뜨므로 이 호출은 호스트 전체의 DPI 인지를 바꾼다. DPI 비인지 앱(AkelPad)은
시스템 확대가 꺼져 100% 로 줄고, **프로세스 DPI 인지는 한 번 정해지면 재시작 전까지 불가역**이라
설정으로도 못 되돌린다(실기 0.17.90 C-4). 최종 해법(사용자 결정) = **아무 DPI 컨텍스트도 설정하지
않는다**: 비인지 호스트에선 Windows 비트맵 확대(크기 정상·약간 흐림), 인지 호스트에선
`GetDpiForWindow` 값으로 우리가 직접 스케일. 스레드 한정 호출(`SetThreadDpiAwarenessContext`)도
선명도만 더할 뿐 필수가 아니라 뺐다 — 단순한 쪽. 프로세스 전역 호출은 **자기 프로세스**
(jamotong.exe 트레이)에서만 정당하다.

IME 팝업(후보창·코드 입력·조합 칩)에 적용하는 법 (2026-09-22):
- `GetDpiForWindow` 는 창이 **지금** 놓인 모니터 기준으로 답한다. 팝업을 목표 자리에 보이지 않게 먼저
  만들거나 옮기고, DPI 를 읽은 뒤 크기를 잡아 띄운다 — 다음 캐럿은 다른 모니터에 있을 수 있으니
  **띄울 때마다 다시 읽는다**. 비인지 호스트에선 96 이 나오므로 100% 기준 치수가 따로 분기 없이 맞는다.
- 사용자가 정한 크기(설정의 글꼴 크기)도 100% 값으로 보고 같은 식으로 배율한다.
- `y < 0` 이 아니라 모니터 **작업영역**(`MonitorFromPoint` + `rcWork`)에 맞춘다. 아래가 모자라면 캐럿
  줄 위로 뒤집는다.
- **고대비**: 고정 팝업 색은 읽히지 않을 수 있다. `SPI_GETHIGHCONTRAST` 가 `HCF_HIGHCONTRASTON` 이면
  `COLOR_WINDOW`/`COLOR_WINDOWTEXT`/`COLOR_GRAYTEXT` 와 `COLOR_HIGHLIGHT`/`COLOR_HIGHLIGHTTEXT` 로 그린다.
  그릴 때마다 읽으면 테마를 바꾼 다음 그리기부터 바로 적용된다.
...
- **구현 요점**:
  ...
  - **★밑줄형 커서 함정(실기 2026-07-24, PuTTY)**: 터미널류는 캐럿 rect가 줄 높이가 아니라
    **밑줄/블록 커서의 두께(몇 px)** 로 보고된다. 그 높이를 글꼴 크기로 쓰면 칩이 극소로
    렌더되고, 캐럿 top에 붙이면 커서를 덮는다. 대응: 캐럿 높이 ≤ 8px면 높이를 버리고
    **커서 폭(터미널 셀 폭)×2**로 줄 높이를 근사(클램프 16~64px), 칩은 **커서 위에 바닥
    정렬**로 띄운다(화면 위로 벗어나면 커서 아래 폴백). 텍스트 캐럿(I-beam)은 줄 높이
    전체로 보고되므로 이 분기는 밑줄 커서에만 걸린다.
...
2. **행동 폴백(강등)** — 플래그는 시험 호스트 2종에서만 검증된 분류자이지 보증이 아니다.
   ...
   컨텍스트를 commit 전용으로 강등한다(포커스 이동 시 재평가). config 킬스위치
   (`InlineComposition=0`)는 분기보다 우선한다. **단, EDIT 계열 검출(§13.1)은 경로
   선택자가 아니다** — commit 경로 '안'의 주입 방식(EM_REPLACESEL vs TSF 삽입)일 뿐이다.
   첫 구현에서 이 검출을 분기보다 선행시켰다가 **Win11 메모장이 통째로 commit 경로로
   떨어졌다**(실기 2026-07-24): 메모장의 편집 컨트롤 클래스가 `RichEditD2DPT`라 이름에
   'edit'가 들어 있고 EM_* 메시지에 응답한다. 클래스명 스니핑은 표준 조합의 1차 대상을
   오분류할 수 있다 — 분기는 항상 문서 상태 플래그가 먼저다. AkelPad류 레거시 EDIT는
   어차피 단명 판정으로 commit이 되므로 선행 가드는 애초에 불필요했다.
...
실기 gate 진행: **메모장(비단명) 인라인 밑줄 조합 실기 PASS(2026-07-24)**. AkelPad(단명) 무회귀와 나머지 세부(백스페이스·Esc·한자·경계키)는 사용 중 관찰을 계속한다.
...
**조합 경계는 코드 경로 하나로.** 포커스 변경, 자판 전환 키(와 preserved key 쌍둥이), 언어바·표시기
전환은 IME에게 같은 사건이다: 남은 음절을 확정하고(키 이벤트 안이면 동기로, 밖이면 기억한 대상에),
**IME가 스스로 누른 모디파이어마다 key-up을 보내고**(Ctrl/Alt를 `SendInput`으로 누른 채 유지하는 모아치기
자판), 리셋한 뒤 팝업을 닫는다. 경로마다 제각각 일부만 하면 언젠가 하나가 key-up을 빠뜨려 호스트에
Ctrl이 눌린 채 남는다. key-up은 확정이 실패해도 보낸다. 2026-09-21 Windows 11 실기: WinForms `TextBox`
에서 전환 키·표시기 클릭·포커스 이동 모두 한 번씩 확정.
...
### 13.6 취소 시 원본 보존 — 커밋 *후* 교체 (교체 가능 호스트 한정)
...
> **갱신(2026-07-24, v0.16.2)**: '먼저 커밋 후 교체'는 **교체가 실제로 되는 호스트**
> (EDIT 계열 `EM_REPLACESEL`, 인라인 조합 TSF range 교체)에서만 안전하다. commit 전용
> 터미널(PuTTY)은 range 편집이 안 되어 커밋한 "가" 뒤에 선택한 한자가 덧붙었다("가家",
> 실기 확인). 그런 호스트는 **커밋하지 않고 조합(칩)을 유지한 채** 후보를 띄우고, 선택
> 시 한자만 삽입(원 음절은 아예 전송 안 됨), 취소 시 조합이 그대로 이어진다 — 원본
> 보존이 '커밋된 텍스트'가 아니라 '살아 있는 조합'으로 달성된다.
>
> **갱신(2026-09-21, v0.23.3)**: 이제 EDIT 계열도 터미널과 같은 길을 탄다 — '커밋 후 교체'는
> 음절이 이미 문서 안의 조합인 **인라인 조합에서만** 남겼다. CUAS 를 거치면 커밋과 뒤따르는 교체가
> 비동기로 전달되어, 교체가 커밋된 원문보다 먼저 닿거나 선택이 옮겨진 뒤 닿아 "원문+한자"가 남거나
> 앞 글자만 바뀔 수 있다. 후보가 정해질 때까지 원문을 살아 있는 조합으로 붙잡아 두면 그 틈이 아예
> 없다. **블록 선택** 변환은 교체 직전에 선택을 다시 읽어, 한자키 때 읽은 텍스트와 다르면(사용자가
> 바꿨다) 교체하지 않는다. 이 검사는 읽기가 믿을 만한 곳(EDIT 직접 읽기)에서만 한다 — 키 이벤트 밖의
> 후보 콜백에서 TSF 읽기는 거부될 수 있고, 빈 결과를 "바뀌었다"로 보면 안 된다. 후보창은 **표시했는지**
> 를 돌려줘야 한다: `CreateWindowEx` 가 실패하면 호출자가 콜백용으로 준비한 것을 정리하고, 원문을
> 보류했으니 잃는 것은 없다. 2026-09-21 Windows 11 실기: WinForms `TextBox`(선택·취소·블록 선택·
> 선택 변경), 메모장 인라인(변화 없음).
...
**쓰이는 중인 파일 업그레이드.** 글자를 치는 모든 프로세스가 TIP DLL 을 물고 있으니 그 위에 `copy /Y` 는
실패한다 — 결과를 안 보는 스크립트는 아무것도 안 바뀌었는데 "완료"를 찍는다. 로드된 파일은 덮어쓸 수
없지만 **이름은 바꿀 수 있다**: 프로그램 파일마다 옆 이름(`name.old.<tag>`)으로 비켜 두고, 새 파일을
복사해 바이트 단위로 대조하고, 하나라도 실패하면 이미 바꾼 파일까지 되돌려 폴더에 판이 섞이지 않게
한다. `*.old.*` 는 다음 실행 때 치운다(아직 매핑된 것은 그 프로세스가 재시작할 때까지 안 지워진다). 등록
뒤에는 `InprocServer32` 를 다시 읽어 방금 설치한 DLL 을 가리키는지 확인하고, `regsvr32` 종료 코드 둘 다
확인한다. 2026-09-21 Windows 11 실기: 사용 중 업그레이드, 3번째 파일 강제 실패 시 앞 두 파일 원복, 같은
스크립트로 되돌리기.
...
| `.jmt` 로더 + 파싱 진단 (`KlayDiag`) — 도구 전용, 입력기 DLL 에는 없다 | `src/klay.c`, `src/hangul_parse.c`, `src/chord_parse.c`, `src/seq_parse.c` |

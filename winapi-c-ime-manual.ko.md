...
*최종 갱신: 2026-07-24 (AkelPad Meta R2 실기 결과·R3 상호작용 시험판)*
...
> 이 유효한 R2 결과와 순서·반복 쓰기를 분리하는 R3 설계는 §12.7.8에 기록한다.
...
display-attribute/trace identity를 쓴다. 빌드와 패키지 검증은 통과했지만 Windows 실기는 아직이므로,
이것을 metadata 가설의 성공 결과로 읽으면 안 된다.
...
#### 12.7.8 정정 R2 실기 결과와 R3 상호작용 시험
...
이를 분리하는 R3는 새 identity와 schema 3을 쓰는 네 profile이다.

| R3 profile | 한 update 안의 metadata plan | 질문 |
|---|---|---|
...
확인한다. 로그에는 실제 속성값·입력 글자·키값·문서 내용·포인터·경로·명령줄을 쓰지 않는다.
화면 결과와 `validation.json`이 모두 있어야 R3 셀을 판정한다.

R3에서 Reading→LANGID도 실패하면 LANGID 존재 자체가 유력해지고, 이것만 통과하면 순서가
유력해진다. Once profile이 첫 키 뒤 Reading-only 수명으로 회복하면 반복 쓰기 효과가 강해지고,
계속 실패하면 첫 LANGID가 context에 지속 상태를 남긴다는 가설이 남는다. 어느 경우도 한 번의
AkelPad 실행만으로 제품의 범용 정책을 정하지 않는다.

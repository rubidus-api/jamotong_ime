...
// 본 프로젝트 형식(ASCII 키 → 초/중/종성 인덱스)으로 독자 인코딩한 것이다. 1차 출처는 공개된 자판
// 배열 자체이며, 제3자 도판·문서의 표현은 옮기지 않았다(RFC-0008 W2-13: 공유-동일조건 자료를 출처로
// 적지 않는다). 정확성은 실제 조합 검증으로 확인: 한글=m f s k g w, 까=k k f, 닭=u f @.
